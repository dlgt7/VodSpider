...
public class Hxq extends Spider {
    ...
    public static String[] decodeUrls(String p0) {
    ...
    public static String fixPicUrl(String p0) {
        if (TextUtils.isEmpty(p0)) {
            return p0;
        }
    ...
    public static String extractPicUrl(JSONObject p0) {
    ...
    public static ArrayList<Vod> parseVod(JSONArray p0, int p1) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            String pic = extractPicUrl(item);
            list.add(new Vod(sid, name, pic, remarks));
        }
        return list;
    }

    public static String resolveUrl(String p0, String p1, String p2, String p3) {
        String url;
    ...
    public static Object[] buildResponse(int code, String body) {
        return new Object[]{
    ...
    public static String rewriteKeyUri(String p0, String p1, String p2, String p3) {
        ...
        sb.append(resolveUrl(p1, p2, p3, uri));
    ...
    public Object[] proxy(Map<String, String> params) {
        ...
        if (TextUtils.isEmpty(url)) {
            return buildResponse(0x190, "Missing url");
        }
        ...
        if ("1".equals(raw)) {
            String[] urls = decodeUrls(url);
            if (urls.length == 0) {
                return buildResponse(0x190, "Missing url");
            }
            String realUrl = fixPicUrl(urls[0].trim());
            if (TextUtils.isEmpty(realUrl)) {
                return buildResponse(0x190, "Bad url");
            }
            ...
            try {
                ...
                return buildResponse(0x1f6, "Fetch failed");
            }
        }
        ...
        if (fmt == 1) {
            ...
            for (int i = 0; i < urls.length; i++) {
                String u = fixPicUrl(urls[i].trim());
            ...
            if (count == 0) {
                return buildResponse(0x1f6, "No valid MP4 URLs");
            }
        ...
        for (int i = 0; i < urls.length; i++) {
            ...
            if (content.contains("#EXT-X-STREAM-INF")) {
                try {
                    ...
                    for (String line : content.split("\\r?\\n")) {
                        ...
                        String newUrl = resolveUrl(baseHost, basePath, baseQuery, trimmed);
            ...
            for (String line : content.split("\\r?\\n")) {
                ...
                if ((trimmed.startsWith("#EXT-X-KEY:") || trimmed.startsWith("#EXT-X-MAP:"))
                    ...
                    resolved = rewriteKeyUri(trimmed, baseHost, basePath, baseQuery);
                } else if (trimmed.startsWith("#")) {
                    resolved = trimmed;
                } else if (trimmed.isEmpty()) {
                    continue;
                } else {
                    resolved = resolveUrl(baseHost, basePath, baseQuery, trimmed);
                }
                sb.append(resolved).append("\n");
            }
        }
        if (allFailed) {
            return buildResponse(0x1f6, "All m3u8 segments failed");
        }
        String result = sb.toString().replace(versionLine,
                "#EXT-X-VERSION:3\n#EXT-X-TARGETDURATION:" + maxDur + "\n#EXT-X-MEDIA-SEQUENCE:0\n")
                + endList;
    ...
    public String fetchScid(String pid) {
        try {
            JSONObject json = new JSONObject();
            json.put("pid", pid);
            json.put("refer", "");
            JSONObject result = fetch(json, "/api/series2/episode/detail");
            if (result == null) {
                return "";
            }
    ...
    public JSONObject fetch(JSONObject json, String path) throws Exception {
        JSONObject result = fetchRaw(json, path);
        if (result == null) {
            return null;
        }
    ...
    public JSONObject fetchRaw(JSONObject json, String path) throws Exception {
        String query;
        if (json.length() == 0) {
            query = "";
        } else {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        JSONObject resp = fetch(json, "/api/series2/arrange/cate");
        if (resp == null) {
            return Result.error("分类加载失败");
        }
        JSONArray seriesList = resp.optJSONArray("seriesList");
        ArrayList<Vod> list = parseVod(seriesList, 0);
        int page;
        try {
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        JSONObject json = new JSONObject();
        json.put("sid", sid);
        JSONObject resp = fetch(json, "/api/series2/detail/normal");
        if (resp == null) {
        ...
        Vod vod = new Vod(sid, series.optString("name", ""), extractPicUrl(series), "");
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        JSONArray seriesList = resp.optJSONArray("seriesList");
        return Result.string(classes, parseVod(seriesList, 0x14), filters);
    }
    ...
    public void init(Context context, String extend) {
        try {
            ...
            fetchRaw(new JSONObject(), "/api/common/configs");
        } catch (Exception e) {
        }
    }
    ...
    public String searchContent(String key, boolean quick) throws Exception {
        ...
        json.put("page", "1");
        json.put("k", key);
        JSONObject resp = fetch(json, "/api/search/s4");
        if (resp == null) {
            return Result.error("搜索无结果");
        }
        JSONArray seriesList = resp.optJSONArray("seriesList");
        return Result.string(parseVod(seriesList, 0));
    }
}
